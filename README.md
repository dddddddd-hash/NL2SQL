...
电商经营数据分析系统（Text2SQL）面向电商运营中的订单统计、商品销量与销售额查询场景，将中文自然语言问题转换为 MySQL 查询，支持按时间、商品等维度进行多表关联与聚合统计，并返回结构化结果和自然语言回答。
系统将订单、商品等业务表的 Schema 与 SQL 示例分别向量化建索引，结合关键词与字段同义词定位相关表和字段；基于 LangGraph 串联检索、Schema Linking、SQL 生成、校验、执行及修复流程，结合字段含义、示例 SQL 与错误反馈组织提示词，仅允许执行 `SELECT` 查询。
## 功能特性
